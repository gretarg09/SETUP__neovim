...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
        if vim.bo.filetype ~= "qf" and vim.bo.buftype == "" then
            keymap('n', '<CR>', smart_enter,
                { buffer = true, noremap = true, silent = true, desc = 'Smart Enter: Open image or toggle fold' })
        end
    end
})

-- overwrite the default obsidian behavior
vim.api.nvim_create_autocmd("User", {
    pattern = "ObsidianNoteEnter",
    callback = function(args)
        vim.keymap.del("n", "<CR>", { buffer = args.buf })
        keymap('n', '<CR>', smart_enter,
            { buffer = args.buf, noremap = true, silent = true, desc = 'Smart Enter: Open image or toggle fold' })
    end
})
